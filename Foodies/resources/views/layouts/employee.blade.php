...
<div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
    <div @click.away="open = false"
        ...
        <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4">
            <a href="#" class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg dark:text-white focus:outline-none focus:shadow-outline">Employee Panel</a>
        ...
        <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
            <x-employee-nav-link :href="route('employee.reservations.index')" :active="request()->routeIs('employee.reservations.index')">
                {{ __('Reservations') }}
            </x-employee-nav-link>
        ...
        <div>
            ...
            @if(session()->has('dark'))
                <div class="p-4 mb-4 text-sm text-gray-700 bg-gray-100 rounded-lg dark:bg-gray-700 dark:text-gray-300" role="alert">
                    <span class="font-medium">Dark!</span> {{ session()->get('dark')}}
                </div>
            @endif
        </div>
        {{ $slot }}
