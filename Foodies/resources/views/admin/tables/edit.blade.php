    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                <a href="{{ route('admin.tables.index') }}" class="px-4 py-2 bg-indigo-50 hover:bg-indigo-200 rounded-lg">
                    Back
                </a>
            </div>
            ...
            <div class="m-2 p-2 bg-white rounded">
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.tables.update', $table->id) }}">
                            ...
                            <div class="mt-1">
                                <select id="location" name="location" class="form-multiselect block w-full mt-1">
                                    @foreach(App\Enums\TableLocation::cases() as $location)
                                        <option value="{{ $location->value }}" @selected($table->location->value == $status->value)>{{ $location->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('loaction')
                        ...
                        <div class="mt-6">
                            <button type="submit" class="px-4 py-2 bg-indigo-50 hover:bg-indigo-200 rounded-lg">Update</button>
                        </div>
                    </form>
                </div>
            </div>
